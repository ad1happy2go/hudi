...
class TestComplexKeyGenExistingTableUpgrade extends HoodieSparkClientTestBase {
  ...
  @Test
  def testUpgradeWithDefaults_NoDuplicates(): Unit = {
    ...
    val auxFile = KeyGenUtils.getComplexKeyEncodingFilePath(basePath)
    val cached = KeyGenUtils.readComplexKeyEncodingFromAuxFile(storage, basePath)
  ...
  @Test
  def testUpgradeWithSafetyNetsOff_ReproducesDuplicates(): Unit = {
    val dataGen = new HoodieTestDataGenerator(0xDEED)
