...
import org.apache.hudi.DataSourceReadOptions.{SPARK_STREAMING_READ_MAX_COMMITS_PER_TRIGGER, INCREMENTAL_READ_HANDLE_HOLLOW_COMMIT}
...
class HoodieStreamSource(
  ...
  val maxCommitsPerBatch: Option[Int] = parameters.get(SPARK_STREAMING_READ_MAX_COMMITS_PER_TRIGGER.key()).map { str =>
