...
public class StorageBasedLockProvider implements LockProvider<StorageLockFile> {
   ...
   * to instantiate it using reflection (0.13.x LP constructor format).
   *
   * @param lockConfiguration The lock configuration, should be transformable into
   *                          StorageBasedLockConfig
   * @param conf              Hadoop config, ignored.
  ...
  // 0.13.1 has no hudi-io StorageSchemes carrying the scheme -> lock-client mapping,
  // so the dispatch lives here. Only GCS is backported; keep class names in sync with
  // the 0.15.1.1 branch so both writers resolve the same client for a given scheme.
  private static @NotNull String getLockServiceClassName(String scheme) {
    if ("gs".equals(scheme)) {
      return "org.apache.hudi.gcp.transaction.lock.GCSStorageLockClient";
    }
