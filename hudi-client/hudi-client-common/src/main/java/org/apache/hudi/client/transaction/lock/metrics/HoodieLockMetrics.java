...
public class HoodieLockMetrics {
  ...
  // Metric names below are backported with StorageBasedLockProvider from the 0.15.1.x patch —
  // keep names identical so dashboards match across both writer versions.
  ...
  public void updateLockExpirationDeadlineMetric(int deadlineMs) {
    if (isMetricsEnabled) {
      // 0.13.1's Metrics.registerGauge is static (no per-instance Metrics handle here).
